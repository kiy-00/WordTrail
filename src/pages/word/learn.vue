...
import WordcardHeader from './components/WordcardHeader.vue'
...
<template>
  <view class="h-screen w-screen flex flex-col justify-between from-[#1b4761] to-[#0e273e] bg-gradient-to-b">
    <!-- Wordcard Header -->
    <WordcardHeader :current-card="currentCard" :total-cards="totalCards" :word="currentWord.word" />

    <!-- Wordcard Content -->
    <scroll-view class="box-border w-full flex-1 overflow-y-auto px-5" scroll-y>
      <view class="font-verdana mb-2 text-left text-4xl text-[#dfe5ea] font-bold">
        {{ currentWord.word }}
      </view>
      <view class="mb-3 text-left text-lg text-[#809bae]">
        {{ currentWord.pronunciation }}
      </view>
      <view v-for="(definition, index) in currentWord.definitions" :key="index" class="font-verdana mb-1 text-left text-lg text-[#d6e6ee]">
      ...
      <view class="font-verdana mt-5 rounded-lg bg-white/20 p-2 text-left">
        ...
        <br>
        <text class="text-lg text-[#d6e6ee]">
